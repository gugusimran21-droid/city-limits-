...
const SpecialOffer = () => {
  ...
  // OPEN MODAL if item has sizes OR top-level toppings (base-price items)
  const handleAddClick = (item) => {
    const hasSizes = Array.isArray(item.sizes) && item.sizes.length > 0;
    const hasTopToppings = Array.isArray(item.toppings) && item.toppings.length > 0;
    if (hasSizes || hasTopToppings) {
