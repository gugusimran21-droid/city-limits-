...
import './Om.css';
import ProductDetailModal from '../Product/ProductDetailModal'; // add file path as needed
import { getItemUnitPrice } from '../../CartContext/CartContext';

const categories = ['CLASSIC PIZZA MENU', 'ALL-TIME FAVOURITE', 'SPECIALITY MENU', 'CREATE YOUR OWN', 'CALZONE', 'GARLIC FINGERS', 'DONAIR', 'NACHOS', 'WINGETTES'];
...
const OurMenu = () => {
  ...
  const handleAddClick = (item) => {
    // if item has sizes, open modal to choose size/toppings
  ...
  const handleModalConfirm = (selectedSize, selectedToppings) => {
    if (!modalProduct) return;
    // call addToCart with options so CartContext constructs proper cart line
    addToCart(modalProduct, 1, { selectedSize, selectedToppings });
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid gap-8 grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3 xl:grid-cols-4">
          {displayItems.map((item, i) => {
            ...
            const isSizeItem = Array.isArray(item.sizes) && item.sizes.length > 0;

            return (
              <div key={item._id} className="relative bg-amber-900/20 rounded-2xl overflow-hidden border border-amber-800/30 backdrop-blur-sm flex flex-col transition-all duration-500" style={{ '--index': i }}>
                  ...
                  <div className="mt-auto flex items-center gap-4 justify-between">
                    <div className="bg-amber-100/10 backdrop-blur-sm px-3 py-1 rounded-2xl shadow-lg">
                      <span className="text-xl font-bold text-amber-300 font-dancingscript">
                        {isSizeItem ? `From ₹${unitPrice.toFixed(2)}` : `₹${unitPrice.toFixed(2)}`}
                      </span>
                    </div>
                    ...
                    <div className="flex items-center gap-2">
                      {quantity > 0 ? (
                        <>
                          <button onClick={() => quantity > 1 ? updateQuantity(cartEntry?._id, quantity - 1) : removeFromCart(cartEntry._id)} className="w-8 h-8 rounded-full bg-amber-900/40 flex items-center justify-center hover:bg-amber-800/50 transition-colors">
                            <FaMinus className="text-amber-100" />
                          </button>
                          <span className="w-8 text-center text-amber-100">{quantity}</span>
                          <button onClick={() => updateQuantity(cartEntry._id, quantity + 1)} className="w-8 h-8 rounded-full bg-amber-900/40 flex items-center justify-center hover:bg-amber-800/50 transition-colors">
                            <FaPlus className="text-amber-100" />
                          </button>
                        </>
                      ) : (
                        <button onClick={() => handleAddClick(item)} className="bg-amber-900/40 px-4 py-1.5 rounded-full font-cinzel text-xs sm:text-sm uppercase tracking-widest transition-transform duration-300 hover:scale-110 hover:shadow-lg hover:shadow-amber-900/20 overflow-hidden border border-amber-800/50">
                          Add to Cart
                        </button>
                      )}
                    </div>
